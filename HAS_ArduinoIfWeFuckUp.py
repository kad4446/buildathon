...
class Card():
	...
	def __init__(self, y, icon):
		...
		self.height = 88
		self.icon = icon
		self.color = (0,158,255)#change to fix palete of buildation
	...
	def update(self):
		self.rect = Rect(self.x, self.y, self.width, self.height)
		if self.on:
			self.color = (0,158,255)
			self.font_color = (255, 255, 255)
		else:
			self.color = (255, 255, 255)
			self.font_color = (0,158,255)
...
class HAS(State):
	...
	def setup(self):
		...
		self.cards = [Card(10 + 98*i, images[i]) for i in range(5)]
		self.background_color = (197, 0, 123)
		...
		self.counter = 0
		self.temp_threshold = 1.0 #will change later
		...
		if self.counter > 10:
			...
			if not data_dict["home"]:
				...
				if self.flag_something_changed:
					...
					self.ser.flush()
					#this somehow works
					number = ret["buffer"] + ret["home"] + ret["win_open"] + ret["door_open"] + ret["light_on"] + ret["temp"] + ret["fan"]
...
if __name__ == '__main__':
	init()
	myfont = font.SysFont("comicsansms", 15)
	screen = display.set_mode((300, 500))
	origin = (150, 250)
	HAS(screen)
